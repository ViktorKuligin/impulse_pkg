...
class goal(Node):
    ...
    def __init__(self):
        ...
        self.sub = self.create_subscription(Vector3, 'direction', self.point_cb, 10)
        # self.sub = self.create_subscription(Point, "point_obj", self.point_cb, 10)
        self.pub_m = self.create_publisher(Marker, "marker_goal", 10)
        self.pub_p = self.create_publisher(Point, "point_goal", 10)
    ...
    def point_cb(self, msg: Point):
        ...
        self.location_x = self.location_x + msg.x
        self.location_y = self.location_y + msg.y
        ...
        msg_hun = Point()
        msg_hun.x = self.location_x
        msg_hun.y = self.location_y
        self.pub_p.publish(msg_hun)
